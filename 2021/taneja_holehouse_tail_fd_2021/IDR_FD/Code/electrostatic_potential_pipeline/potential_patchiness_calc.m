function output = potential_patchiness_calc(run_parallel)
    
    input_file = '../../Processed_Data/phi_patchiness_input.csv';
    output_dir = '../../Processed_Data/Phi_Patchiness_Data';
...
function [ppatches, npatches, colnames]= find_patches(G_connected,X_subset,pcutoff,ncutoff,idr_x, idr_y, idr_z)
    ...
    for i = 1:length(unique_pos_components)
        ...
        if patch_size_percentage > .01
            ...
            patch_centroid_nearest_coord = X_subset(patch_centroid_idx_in_X_subset,1:3); %might be useful to track if there is not a node that is actually nearby to the patch centroid -- (think of a sphere that is 100% patchy, centroid will be in the origin of that sphere and this is not near to any node on surface)
...
function [ppatches_dist_distribution, npatches_dist_distribution, colnames]= patch_accessibility(G_connected,X_subset,pcutoff,ncutoff,idr_x, idr_y, idr_z)
    ...
    %basic idea: for each patch, sample 10% of its points and then
    %calculate the surface distance between each of those points and the
    %IDR junction
    ...
    for i = 1:length(unique_pos_components)
        ...
        if patch_size_percentage > .01
            ...
            pos_nodes_index_in_X_subset = pos_nodes(pos_component_index_sample); %map indices from X_subset_pos to X_subset
    ...
    for i = 1:length(unique_neg_components)
        ...
        if patch_size_percentage > .01
            ...
            neg_nodes_index_in_X_subset = neg_nodes(neg_component_index_sample); %map indices from X_subset_neg to X_subset

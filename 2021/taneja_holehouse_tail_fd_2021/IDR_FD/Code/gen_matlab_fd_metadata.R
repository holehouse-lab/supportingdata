...
#this is done because sometimes PDB and fasta do not match
...
for (i in 1:nrow(fd_candidate_data))
{
  ...
  for (curr_chain in chain_list)
  {
    ...
    idr_charge_data = idr_charge_stats %>% filter(Uniprot_ID == curr_uniprot_id) %>% filter(N_Terminal_Status == n_terminal) %>% as.data.frame()
    
    fp = signif(idr_charge_data[,'fp'],2)
    fn = signif(idr_charge_data[,'fn'],2)
    ncpr = signif(idr_charge_data[,'ncpr'],2)
    tcpr = signif(idr_charge_data[,'tcpr'],2)
    ...
    pdb_filename = paste0('../PDB_Structure_FD_candidates_within10/PDB_Files/',paste0(curr_pdb_id,'_',curr_chain,'.pdb'))
    ...
    if ((length(unique(pdb_data$atom$chain)) == 1) && length(unique(pdb_data$atom$resno)) > 10) 
    {
      ...
      if (nrow(relevant_atom_data) > 0)
      {
        ...
        if (abs(idr_start - fd_actual_start) <= 10)
        {
          ...
          matlab_metadata = bind_rows(matlab_metadata, data.frame(PDB_ID = curr_pdb_id,
                                                                  chain = curr_chain,
                                                                  fp = fp,
                                                                  fn = fn,
                                                                  ncpr = ncpr,
          ...
          for (j in 1:5)
          {
            relevant_atom_data_nearest_residue = relevant_atom_data %>% filter(abs(resno_updated-fd_actual_start) == 30*j) %>% filter(elety == 'CA') %>% as.data.frame()
            ...
            matlab_metadata = bind_rows(matlab_metadata, data.frame(PDB_ID = curr_pdb_id,
                                                                    ...
                                                                    fd_coverage_percentage = fd_coverage_percentage,
                                                                    x_loc = x_loc, y_loc = y_loc, z_loc = z_loc, 
                                                                    type = paste0(30*j,'_from_nearest')))
...
matlab_metadata_unique = matlab_metadata_unique %>% group_by(PDB_ID, type) %>% filter(FD_IDR_terminal_dist_real == min(FD_IDR_terminal_dist_real)) %>% as.data.frame()
matlab_metadata_unique = matlab_metadata_unique %>% group_by(PDB_ID, type) %>% filter(chain == min(chain)) %>% as.data.frame()
